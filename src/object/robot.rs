...
impl Robot {
    pub fn new(id: String, pose: (f32, f32, f32), vel: (f32, f32), lidar_present: bool, footprint: Vec<f32>) -> Robot {
        
        if footprint.len() == 1
        {
            let fshape = Footprint::Circular(Ball { radius: (footprint[0])});
            ...
            let fshape = Footprint::Rectangular(Cuboid { half_extents: Vector::new(width, height)});
    ...
    fn draw(&self, tf: fn((f32, f32)) -> (f32, f32)) {
        ...
        match self.shape
        {
            ...
            Footprint::Rectangular(r) =>
            {
                ...
                // Draw the body
                draw_triangle(Vec2 { x: tf_p1.0, y: tf_p1.1 }, Vec2 { x: tf_p2.0, y: tf_p2.1 }, Vec2 { x: tf_p3.0, y: tf_p3.1 }, BLACK);
                draw_triangle(Vec2 { x: tf_p1.0, y: tf_p1.1 }, Vec2 { x: tf_p3.0, y: tf_p3.1 }, Vec2 { x: tf_p4.0, y: tf_p4.1 }, BLACK);
