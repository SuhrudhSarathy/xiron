            ...
            Ok(msg) => {
                if msg.type_url == "vel" {
                    ...
                    match twist_msg
                    {
                        ...
                        Err(_) => {
                            return Err(CommResponseError { reason: "Could not decode TwistMsg".to_string() });
                        },
                    }
                    ...
                    match pose_msg
                    {
                        ...
                        Err(_) => {
                            return Err(CommResponseError { reason: "Could not decode PoseMsg".to_string() });
                        },
                    }
                }
                    ...
                    return Err(CommResponseError {reason: format!("Unknown msg_type in Protobuf message: {}", msg.type_url).to_string()});
                }
            },
            Err(_error) => {
                return Err(CommResponseError { reason: "Could not decode Protobuf".to_string() });
            }
        }
    }
...
{
    pub fn new(url: &'static str, sim_to_client_port: u16, client_to_sim_port: u16) -> XironCommServer
    {
        XironCommServer { url, sim_to_client_port, client_to_sim_port }
    }

    pub fn start(self) -> (Sender<Result<CommResponse, CommResponseError>>, Receiver<Result<CommResponse, CommResponseError>>)
    {
        let url = self.url;
        ...
        thread::spawn(move || {
            ...
            let sim_to_client_server = TcpListener::bind(&sim_to_client_url).expect(format!("Could not bind Sim2Client server at url: {}", sim_to_client_url).as_str());
            println!("Successfull binded to {}", sim_to_client_url);

            let client_to_sim_server = TcpListener::bind(&client_to_sim_url).expect(format!("Could not bind Client2Sim server at url: {}", client_to_sim_url).as_str());
            println!("Successfull binded to {}", client_to_sim_url);

            let sim_to_client_thread_builder = thread::Builder::new().name("sim2clientServer".to_string());
            let client_to_sim_thread_builder = thread::Builder::new().name("client2simServer".to_string());


            let s2c_thread = sim_to_client_thread_builder.spawn(move || { 
                for stream in sim_to_client_server.incoming() {
                    ...
                    thread::spawn(
                    {
                        ...
                        move || {
                            handler.handle_sending_messages(stream, sim_to_client_receiver_clone);
                    }});
                }
            ...
            let c2s_thread = client_to_sim_thread_builder.spawn(move || {
                for stream in client_to_sim_server.incoming() {
                    ...
                    thread::spawn({
                        ...
                        move || {
                            handler.handle_receiving_messages(stream, client_to_sim_sender_clone);
                    }});
                }
            }).unwrap();
    ...
    fn handle_receiving_messages(&self, stream: TcpStream, sender: Sender<Result<CommResponse, CommResponseError>>)
    {
        let callback = |req: &Request, response: Response| {
            println!("New connection: {} for receiving messages", req.uri().path());
            Ok(response)
        };
    ...
    fn handle_sending_messages(&self, stream: TcpStream, reciever: Receiver<Result<CommResponse, CommResponseError>>) {
        let callback = |req: &Request, response: Response| {
            println!("New connection recieved from path: {} for Sending Messages", req.uri().path());
            Ok(response)
        };
        ...
        match websocket
        {
            ...
            Err(e) => {
                println!("Recieved Server handshake error while trying to send data: {}", e);
            },
        }
    }
}
