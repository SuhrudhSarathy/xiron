...
pub trait Collidable
{
    ...
    fn collision_check(&self, other: &dyn Collidable)->bool {
        ...
        let pos1 = Isometry::new(Vector::new(self.get_pose().0, self.get_pose().1), self.get_pose().2);
        let pos2 = Isometry::new(Vector::new(other.get_pose().0, other.get_pose().1), other.get_pose().2);
    ...
    fn raycast(&self, ray: &Ray)->f32
    {
        ...
        let ray_result = shape.cast_local_ray(ray, 5.0, true);
        match ray_result
        {
            None => {return -10.0},
            Some(r) => {return r}
